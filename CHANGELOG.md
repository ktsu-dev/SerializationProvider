## v1.0.8 (patch)

Changes since v1.0.7:

- Sync .github\workflows\dotnet.yml ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Sync global.json ([@ktsu[bot]](https://github.com/ktsu[bot]))

## v1.0.7 (patch)

Changes since v1.0.6:

- Sync .github\workflows\dotnet.yml ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Sync .github\dependabot.yml ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Sync .gitignore ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Sync .gitattributes ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Sync global.json ([@ktsu[bot]](https://github.com/ktsu[bot]))
- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.7-pre.1 (prerelease)

No significant changes detected since v1.0.7.

## v1.0.6 (patch)

Changes since v1.0.5:

- Use Ensure.NotNull for argument validation ([@Claude](https://github.com/Claude))
- Use a primary constructor in VstPresetSerializationProvider ([@Claude](https://github.com/Claude))
- Add VST3 .vstpreset interchange codec and provider backend ([@Claude](https://github.com/Claude))

## v1.0.6-pre.33 (prerelease)

Changes since v1.0.6-pre.32:

- Bump Polyfill from 10.8.0 to 10.8.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))
- Bump the ktsu group with 3 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.32 (prerelease)

Changes since v1.0.6-pre.31:

- Bump Polyfill from 10.7.1 to 10.8.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.31 (prerelease)

Changes since v1.0.6-pre.30:

- Bump Polyfill from 10.7.0 to 10.7.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.30 (prerelease)

Changes since v1.0.6-pre.29:

- Bump Polyfill from 10.6.0 to 10.7.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.29 (prerelease)

Changes since v1.0.6-pre.28:

- Bump Polyfill from 10.5.1 to 10.6.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.28 (prerelease)

Changes since v1.0.6-pre.27:

- Bump MSTest.Sdk from 4.2.2 to 4.2.3 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.27 (prerelease)

Changes since v1.0.6-pre.26:

- Bump FluentAssertions from 8.9.0 to 8.10.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))
- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.26 (prerelease)

Changes since v1.0.6-pre.25:

- Bump Polyfill from 10.5.0 to 10.5.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.25 (prerelease)

Changes since v1.0.6-pre.24:

- Bump Polyfill from 10.4.0 to 10.5.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.24 (prerelease)

Changes since v1.0.6-pre.23:

- Bump Polyfill from 10.3.0 to 10.4.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.23 (prerelease)

Changes since v1.0.6-pre.22:

- Bump MSTest.Sdk from 4.2.1 to 4.2.2 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.22 (prerelease)

Changes since v1.0.6-pre.21:

- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.21 (prerelease)

Changes since v1.0.6-pre.20:

- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.20 (prerelease)

Changes since v1.0.6-pre.19:

- Bump Polyfill from 10.1.1 to 10.3.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.19 (prerelease)

Changes since v1.0.6-pre.18:

- Bump Polyfill from 10.0.0 to 10.1.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.18 (prerelease)

Changes since v1.0.6-pre.17:

- Bump MSTest.Sdk from 4.1.0 to 4.2.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.17 (prerelease)

Changes since v1.0.6-pre.16:

- Bump Polyfill from 9.24.0 to 10.0.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.16 (prerelease)

Changes since v1.0.6-pre.15:

- Bump Polyfill from 9.23.0 to 9.24.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.15 (prerelease)

Changes since v1.0.6-pre.14:

- Bump Polyfill from 9.22.0 to 9.23.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.14 (prerelease)

Changes since v1.0.6-pre.13:

- Bump FluentAssertions from 8.8.0 to 8.9.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.13 (prerelease)

Changes since v1.0.6-pre.12:

- Bump Polyfill from 9.18.0 to 9.22.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.12 (prerelease)

Changes since v1.0.6-pre.11:

- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.11 (prerelease)

Changes since v1.0.6-pre.10:

- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.10 (prerelease)

Changes since v1.0.6-pre.9:

- Bump Polyfill from 9.17.0 to 9.18.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.9 (prerelease)

Changes since v1.0.6-pre.8:

- Bump Polyfill from 9.16.0 to 9.17.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.8 (prerelease)

Changes since v1.0.6-pre.7:

- Bump Polyfill from 9.13.0 to 9.16.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.7 (prerelease)

Changes since v1.0.6-pre.6:

- Bump Polyfill from 9.12.0 to 9.13.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.6 (prerelease)

Changes since v1.0.6-pre.5:

- Bump Polyfill from 9.11.0 to 9.12.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.5 (prerelease)

Changes since v1.0.6-pre.4:

- Bump Polyfill from 9.10.0 to 9.11.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.4 (prerelease)

Changes since v1.0.6-pre.3:

- Bump Polyfill from 9.9.0 to 9.10.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.3 (prerelease)

Changes since v1.0.6-pre.2:

- Bump Polyfill from 9.8.1 to 9.9.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.2 (prerelease)

Changes since v1.0.6-pre.1:

- Bump the microsoft group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.6-pre.1 (prerelease)

No significant changes detected since v1.0.6.

## v1.0.5 (patch)

Changes since v1.0.4:

- Add optional NuGet API key parameter to pipeline configuration ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.4 (patch)

Changes since v1.0.3:

- Compatibility supressions ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.3 (patch)

Changes since v1.0.2:

- migrate to dotnet 10 ([@matt-edmondson](https://github.com/matt-edmondson))
- Add deprecation notice to README ([@matt-edmondson](https://github.com/matt-edmondson))
- Update package versions and improve release notes handling in PSBuild script ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.3-pre.2 (prerelease)

Changes since v1.0.3-pre.1:

- Update package versions and improve release notes handling in PSBuild script ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.3-pre.1 (prerelease)

No significant changes detected since v1.0.3.

## v1.0.2 (patch)

Changes since v1.0.1:

- Refactor namespaces to include 'ktsu' prefix for SerializationProvider and its extensions in source and test files. ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.1 (patch)

Changes since v1.0.0:

- Enhance Invoke-DotNetPack function to escape newlines in release notes for MSBuild compatibility ([@matt-edmondson](https://github.com/matt-edmondson))

## v1.0.0 (major)

- Remove SerializationException and DeserializationException classes; update test project by removing Moq package reference. ([@matt-edmondson](https://github.com/matt-edmondson))
- Initial implementation ([@matt-edmondson](https://github.com/matt-edmondson))

